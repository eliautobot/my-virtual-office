...
const FURNITURE_BOUNDS = {
    ...
    'bossDesk':      { w: 110, h: 75,  ox: 0.5,  oy: 0.47 },  // translate(x,y), -55,-35 to 55,40
    ...
    'meetingTable':  { w: 200, h: 120, ox: 0,    oy: 0    },
...
function drawWeatherOnWindow(wx, wy, ww, wh, isLeft) {
    ...
    if (cond === 'sunny') {
        if (isDaytime) {
            ...
            if (isLeft) {
                ...
                grad.addColorStop(0, 'rgba(255,245,157,0.3)');
                grad.addColorStop(0.5, 'rgba(255,235,59,0.1)');
    ...
    // ─── PARTLY CLOUDY ───
        ...
        _drawCloud(wx + (drift + 5) % (ww + 10) - 5, wy + 7, 14, 5, 0.25);
        _drawCloud(wx + (drift * 0.7 + ww * 0.6) % (ww + 10) - 5, wy + 12, 10, 4, 0.2);
    ...
    // ─── OVERCAST ───
        ...
        ctx.beginPath(); ctx.rect(wx, wy, ww, wh); ctx.clip();
        // Grey blanket
        ctx.fillStyle = 'rgba(120,120,125,0.15)';
        ctx.fillRect(wx, wy, ww, wh);
        ...
        ctx.fillStyle = 'rgba(140,140,145,0.3)';
        ctx.fillRect(wx + (drift2) % ww - 5, wy + 3, 16, 5);
        ctx.fillRect(wx + (drift2 + 20) % ww - 5, wy + 5, 12, 4);
        // Lower layer — lighter, faster
        _drawCloud(wx + (drift1 + 8) % (ww + 10) - 5, wy + 10, 14, 5, 0.2);
        _drawCloud(wx + (drift1 * 0.8 + ww * 0.5) % (ww + 10) - 5, wy + 16, 11, 4, 0.18);
        _drawCloud(wx + (drift1 * 1.2 + ww * 0.3) % (ww + 10) - 5, wy + 22, 9, 3, 0.15);
        // Dim the whole window slightly
        ctx.fillStyle = 'rgba(80,80,85,0.06)';
        ctx.fillRect(wx, wy, ww, wh);
        ctx.restore();
    ...
    // ─── DRIZZLE ───
        ...
        ctx.beginPath(); ctx.rect(wx, wy, ww, wh); ctx.clip();
        // Slight grey tint
        ctx.fillStyle = 'rgba(130,130,135,0.1)';
        ctx.fillRect(wx, wy, ww, wh);
        // Tiny thin rain lines — sparse, slow
        ctx.strokeStyle = 'rgba(160,195,220,0.4)';
        ctx.lineWidth = 0.5;
    ...
    // ─── RAIN ───
        ...
        ctx.save();
        ctx.beginPath(); ctx.rect(wx, wy, ww, wh); ctx.clip();
        ctx.fillStyle = 'rgba(80,85,95,0.18)';
        ctx.fillRect(wx, wy, ww, wh);
        // Dense rain streaks
        var rWindAngle = Math.min(wind * 0.025, 0.5);
        ctx.strokeStyle = 'rgba(130,180,220,0.6)';
        ctx.lineWidth = 1;
        ...
        // Clouds
        _drawCloud(wx + (_weatherTick * 0.03 + 5) % (ww + 10) - 5, wy + 5, 14, 5, 0.3);
        ctx.restore();
    ...
    // ─── HEAVY RAIN ───
        ...
        ctx.strokeStyle = 'rgba(140,190,230,0.35)';
        ctx.lineWidth = 1.5;
    ...
    // ─── THUNDERSTORM ───
        ...
        ctx.strokeStyle = 'rgba(120,165,200,0.65)';
        ctx.lineWidth = 1;
        ...
        _drawCloud(wx + (stDrift + 3) % (ww + 10) - 5, wy + 5, 16, 6, 0.4);
        _drawCloud(wx + (stDrift * 0.6 + ww * 0.5) % (ww + 10) - 5, wy + 10, 12, 5, 0.35);
        // Splashes
    ...
    // ─── FOGGY ───
        ...
        // Background haze
        ctx.fillStyle = 'rgba(195,195,200,0.2)';
        ctx.fillRect(wx, wy, ww, wh);
        ...
        ctx.fillStyle = 'rgba(220,220,225,0.15)';
    ...
    // ─── SNOW ───
        ...
        ctx.fillStyle = 'rgba(240,242,248,0.4)';
...
const LOCATIONS = {
    ...
    meeting: { x: 370, y: 440, w: 260, h: 120 },
    lounge: { x: 60, y: 550 },
    cooler: { x: 820, y: 540 },
    ...
    interactions: {
        windows: [
            { x: 406, y: 74 },    // HQ left window
            { x: 594, y: 74 },    // HQ right window
        ],
        couchSeats: [
...
const MEETING_SLOTS = (() => {
    ...
    return [
        { x: m.x + 35, y: m.y + 5 },    // top row
        { x: m.x + 85, y: m.y + 5 },
        { x: m.x + 135, y: m.y + 5 },
        { x: m.x + 185, y: m.y + 5 },
        { x: m.x + 235, y: m.y + 5 },
        { x: m.x + 35, y: m.y + 105 },  // bottom row
        { x: m.x + 85, y: m.y + 105 },
        { x: m.x + 135, y: m.y + 105 },
        { x: m.x + 185, y: m.y + 105 },
        { x: m.x + 235, y: m.y + 105 },
...
const FURNITURE_ACTIONS = {
    ...
    'tv':             { spots: [{ dx: 0, dy: 38, faceDir: 1 }], action: 'watch'  },
    'couch':          {
        spots: [
            { dx: 18, dy: 25, faceDir: 1, action: 'sit' },
            { dx: 18, dy: 48, faceDir: 1, action: 'sit' },
...
function getInteractionSpots() {
    ...
    officeConfig.furniture.forEach(function(item) {
        ...
        (fa.spots || []).forEach(function(spot) {
            ...
            var ws = { x: item.x + spot.dx, y: item.y + spot.dy };
...
class Agent {
    ...
    draw() {
        ...
        if (this.state === 'working' && this.isSitting && !isMoving) {
            ...
            // Ping pong — one arm holds racket, swings on hit
            ...
            var racketArm = this.faceDir; // arm facing the table
            var swingAngle = _swing * 0.8; // 0 to 0.8 radians
            ctx.fillStyle = this.color;
            if (racketArm === 1) {
                ...
                ctx.fillStyle = '#2196f3'; // paddle face
                ctx.beginPath(); ctx.arc(-1, 16, 4, 0, Math.PI * 2); ctx.fill();
                ctx.fillStyle = '#1976d2'; // paddle edge
...
function drawBossDesk(x, y) {
    ctx.save(); ctx.translate(x, y);
    ctx.fillStyle = 'rgba(255,215,0,0.1)'; ctx.fillRect(-55, -35, 110, 75);
    ctx.strokeStyle = 'rgba(255,215,0,0.3)'; ctx.strokeRect(-55, -35, 110, 75);
    ctx.fillStyle = '#5d4037'; ctx.fillRect(-45, -25, 90, 50);
    ctx.fillStyle = '#8d6e63'; ctx.fillRect(-42, -22, 84, 44);
    ctx.fillStyle = '#263238'; ctx.fillRect(-35, -50, 30, 22); ctx.fillRect(5, -50, 30, 22);
    ctx.fillStyle = '#4fc3f7'; ctx.fillRect(-32, -47, 24, 16); ctx.fillStyle = '#81d4fa'; ctx.fillRect(8, -47, 24, 16);
    ctx.fillStyle = '#333'; ctx.fillRect(-10, -28, 20, 5);
    ctx.fillStyle = '#37474f'; ctx.fillRect(-18, -18, 36, 8);
    ctx.restore();
}
...
function drawMeetingRoom(x, y) {
    // Meeting table: 200px wide (5 tiles), 120px tall
    var tw = 200, th = 120;
    var pad = 20;
    var tableX = x + pad, tableY = y + 28;
    var tableW = tw - pad * 2, tableH = 60;
    var UNUSED_m = LOCATIONS.meeting;
    ...
    // Items on table
    ctx.fillStyle = '#fff'; ctx.fillRect(x + 40, tableY + 8, 14, 18);
    ctx.fillStyle = '#e0e0e0'; ctx.fillRect(x + 42, tableY + 10, 10, 4);
    ctx.fillStyle = '#263238'; ctx.fillRect(x + 70, tableY + 6, 28, 18);
    ctx.fillStyle = '#4fc3f7'; ctx.fillRect(x + 72, tableY + 8, 24, 12);
    ctx.fillStyle = '#455a64'; ctx.fillRect(x + 70, tableY + 22, 28, 3);
    ctx.fillStyle = '#fff';
    ctx.fillRect(x + 28, tableY + 16, 5, 6);
    ctx.fillRect(x + tw - 48, tableY + 16, 5, 6);

    // Chairs — 5 per side, evenly spaced
    var chairSpacing = (tableW - 14) / 4;
    for (var i = 0; i < 5; i++) {
        var cx = tableX + i * chairSpacing;
        ctx.fillStyle = '#37474f'; ctx.fillRect(cx, y + 8, 14, 14);
        ctx.fillStyle = '#455a64'; ctx.fillRect(cx + 1, y + 9, 12, 12);
        ctx.fillStyle = '#546e7a'; ctx.fillRect(cx + 2, y + 10, 10, 6);
        ctx.fillStyle = '#37474f'; ctx.fillRect(cx, y + 96, 14, 14);
        ctx.fillStyle = '#455a64'; ctx.fillRect(cx + 1, y + 97, 12, 12);
        ctx.fillStyle = '#546e7a'; ctx.fillRect(cx + 2, y + 98, 10, 6);
    }
...
function drawTV(x, y) {
    ...
    // Standby screen
    ctx.fillStyle = '#1a237e'; ctx.fillRect(x + 5, y + 5, 40, 22);
    ctx.fillStyle = 'rgba(255,255,255,0.05)'; ctx.fillRect(x + 5, y + 5, 40, 11);
    // Stand
...
function updatePongGames() {
    ...
    for (var i = pongGames.length - 1; i >= 0; i--) {
        ...
        if (g.phase === 'playing') {
            ...
            // Move agents to track ball vertically
...
function drawPongGames() {
    for (var i = 0; i < pongGames.length; i++) {
        ...
        ctx.fillRect(tx - 25, ty - 34, 50, 12);
        ctx.font = '7px Arial'; ctx.textAlign = 'center';
        ctx.fillStyle = '#f44336';
        ctx.fillText(g.p1.name + ':' + g.p1Score, tx - 10, ty - 25);
        ctx.fillStyle = '#2196f3';
        ctx.fillText(g.p2.name + ':' + g.p2Score, tx + 12, ty - 25);
...
const GATHERING_SPOTS = [
    { x: 120, y: 590, label: 'lounge', capacity: 99 },
    { x: 500, y: 620, label: 'entrance', capacity: 99 },
    { x: 450, y: 350, label: 'hallway', capacity: 99 },
    { x: 820, y: 560, label: 'break room', capacity: 99 },
];
...
function startGathering() {
    ...
    // Pick a spot
    const spot = GATHERING_SPOTS[Math.floor(Math.random() * GATHERING_SPOTS.length)];
    const gathering = {
...
function loop() {
    ...
    _perfStart('agents');
    agents.forEach(a => { a.update(); maybeThrowAirplane(a); maybeStartRPS(a); maybeStartSocial(a); maybeStartDarts(a); maybeStartPong(a); });
    var _behindWalls = [];
    var _frontWalls = [];
...
function drawEditOverlay() {
    ...
    if (placingType && _ghostPos) {
        ...
        var ghostBounds = FURNITURE_BOUNDS[placingType] || { w: TILE, h: TILE, ox: 0, oy: 0 };
        var _gbOX = ghostBounds.ox || 0;
        var _gbOY = ghostBounds.oy || 0;
        // Visual top-left corner (accounts for origin offset)
        var _visX = _ghostPos.x - _gbOX * ghostBounds.w;
        var _visY = _ghostPos.y - _gbOY * ghostBounds.h;
        ...
        var _gEndTX = Math.floor((_visX + ghostBounds.w - 1) / TILE);
        var _gEndTY = Math.floor((_visY + ghostBounds.h - 1) / TILE);
        ...
        ctx.lineWidth = 2;
        ctx.strokeRect(_visX, _visY, ghostBounds.w, ghostBounds.h);

        // Show red X if invalid
        if (!_placementValid) {
            ctx.fillStyle = 'rgba(244, 67, 54, 0.6)';
            ctx.fillRect(_visX, _visY, ghostBounds.w, ghostBounds.h);
            ctx.fillStyle = '#fff';
            ctx.font = 'bold 14px Arial';
            ctx.textAlign = 'center';
            ctx.fillText('✕', _visX + ghostBounds.w / 2, _visY + ghostBounds.h / 2 + 5);
        }
    }
    ...
    if (selectedItemId) {
        ...
        if (selItem) {
            var selBounds = FURNITURE_BOUNDS[selItem.type] || { w: TILE, h: TILE, ox: 0, oy: 0 };
            var _selOX = selBounds.ox || 0;
            var _selOY = selBounds.oy || 0;
            var _selVisX = selItem.x - _selOX * selBounds.w;
            var _selVisY = selItem.y - _selOY * selBounds.h;
            // Highlight ALL tiles
            var _sTX1 = Math.max(0, Math.floor(_selVisX / TILE));
            var _sTY1 = Math.max(0, Math.floor(_selVisY / TILE));
            var _sTX2 = Math.floor((_selVisX + selBounds.w - 1) / TILE);
            var _sTY2 = Math.floor((_selVisY + selBounds.h - 1) / TILE);
            ...
            ctx.strokeStyle = '#ffd600';
            ctx.lineWidth = 2;
            ctx.strokeRect(_selVisX - 3, _selVisY - 3, selBounds.w + 6, selBounds.h + 6);
            ctx.setLineDash([]);
        }
    }
...
function _itemOverlaps(type, x, y, excludeId) {
    ...
    for (var i = 0; i < officeConfig.furniture.length; i++) {
        ...
        var v2 = _visualTopLeft(item.type, item.x, item.y);
...
function _findFurnitureAt(wx, wy) {
    ...
    for (var i = officeConfig.furniture.length - 1; i >= 0; i--) {
        var item = officeConfig.furniture[i];
        var b = FURNITURE_BOUNDS[item.type] || { w: TILE, h: TILE };
        if (wx >= item.x && wx <= item.x + b.w && wy >= item.y && wy <= item.y + b.h) {
            return item;
        }
    }
    return null;
...
function _updateFloatingToolbarPosition() {
    ...
    var bounds = FURNITURE_BOUNDS[selItem.type] || { w: TILE, h: TILE };
    var base = getBaseScale();
    var totalZoom = base * camera.zoom;
    var rect = canvas.getBoundingClientRect();
    // World center-top → display coords
    var wx = selItem.x + bounds.w / 2;
    var wy = selItem.y;
    var dx = (wx - W / 2 - camera.x) * totalZoom + displayW / 2;
    var dy = (wy - H / 2 - camera.y) * totalZoom + displayH / 2;
    // Display → screen CSS
        ...
        if (isDragging && selectedItemId && !placingType) {
            ...
            for (var _di = 0; _di < officeConfig.furniture.length; _di++) {
                if (officeConfig.furniture[_di].id === selectedItemId) {
                    ...
                    // Highlight matches item's visual footprint (accounting for origin offset)
                    var _dox = dBounds.ox || 0, _doy = dBounds.oy || 0;
                    _editDragTileHighlight = {
                        x: snapX - _dox * dBounds.w,
                        y: snapY - _doy * dBounds.h,
                        w: dBounds.w, h: dBounds.h
                    };
